...
createregistration =
  "CREATE TABLE IF NOT EXISTS registration (id INT PRIMARY KEY, user INT NOT NULL, course INT NOT NULL)";
/* (1, user #4, course #2) */
createposts =
  "CREATE TABLE IF NOT EXISTS posts (id INT PRIMARY KEY, author INT NOT NULL, message INT NOT NULL, likes INT DEFAULT 0, comments INT DEFAULT 0)";
createmessages =
  "CREATE TABLE IT NOT EXISTS messages (id INT PRIMARY KEY, author INT NOT NULL, text MEDIUMTEXT NOT NULL, date DATETIME NOT NULL, postid = INT NOT NULL)";
createthreads =
...
/* match all users to the threads they follow; find all courses using above query^ then find all threads based on those course ids using for loop */
...
messages = "SELECT text FROM messages WHERE id=?";

messagesbyauthor = "SELECT * FROM messages WHERE author=?";
